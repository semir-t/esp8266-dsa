    ...
    switch(evt->event_id) {
        ...
        case HTTP_EVENT_DISCONNECTED:
            ...
            if (err != 0) {
                ...
                printDEBUG(DHTTP, "Last esp error code: 0x%x", err);
                printDEBUG(DHTTP, "Last mbedtls failure: 0x%x", mbedtls_err);
            }
            break;
    }
    return ESP_OK;
...
static void http_rest_with_url(void)
{
    ...
    err = esp_http_client_perform(client);
    if (err == ESP_OK) {
        printDEBUG(DHTTP, "HTTP PUT Status = %d, content_length = %d",
                esp_http_client_get_status_code(client),
                esp_http_client_get_content_length(client));
    } else {
    ...
    err = esp_http_client_perform(client);
    if (err == ESP_OK) {
        printDEBUG(DHTTP, "HTTP PATCH Status = %d, content_length = %d",
                esp_http_client_get_status_code(client),
                esp_http_client_get_content_length(client));
    } else {
    ...
    err = esp_http_client_perform(client);
    if (err == ESP_OK) {
        printDEBUG(DHTTP, "HTTP DELETE Status = %d, content_length = %d",
                esp_http_client_get_status_code(client),
                esp_http_client_get_content_length(client));
    } else {
    ...
    err = esp_http_client_perform(client);
    if (err == ESP_OK) {
        printDEBUG(DHTTP, "HTTP HEAD Status = %d, content_length = %d",
                esp_http_client_get_status_code(client),
                esp_http_client_get_content_length(client));
    } else {
...
static void http_auth_basic(void)
{
    ...
    if (err == ESP_OK) {
        printDEBUG(DHTTP, "HTTP Basic Auth Status = %d, content_length = %d",
                esp_http_client_get_status_code(client),
                esp_http_client_get_content_length(client));
    } else {
...
static void http_auth_basic_redirect(void)
{
    ...
    if (err == ESP_OK) {
        printDEBUG(DHTTP, "HTTP Basic Auth redirect Status = %d, content_length = %d",
                esp_http_client_get_status_code(client),
                esp_http_client_get_content_length(client));
    } else {
        ESP_LOGE(TAG, "Error perform http request %s", esp_err_to_name(err));
...
static void http_auth_digest(void)
{
    ...
    if (err == ESP_OK) {
        printDEBUG(DHTTP, "HTTP Digest Auth Status = %d, content_length = %d",
                esp_http_client_get_status_code(client),
                esp_http_client_get_content_length(client));
    } else {
        ESP_LOGE(TAG, "Error perform http request %s", esp_err_to_name(err));
...
static void https_with_url(void)
{
    ...
    if (err == ESP_OK) {
        printDEBUG(DHTTP, "HTTPS Status = %d, content_length = %d",
                esp_http_client_get_status_code(client),
                esp_http_client_get_content_length(client));
    } else {
        ESP_LOGE(TAG, "Error perform http request %s", esp_err_to_name(err));
...
static void http_relative_redirect(void)
{
    ...
    if (err == ESP_OK) {
        printDEBUG(DHTTP, "HTTP Relative path redirect Status = %d, content_length = %d",
                esp_http_client_get_status_code(client),
                esp_http_client_get_content_length(client));
    } else {
        ESP_LOGE(TAG, "Error perform http request %s", esp_err_to_name(err));
...
static void http_absolute_redirect(void)
{
    ...
    if (err == ESP_OK) {
        printDEBUG(DHTTP, "HTTP Absolute path redirect Status = %d, content_length = %d",
                esp_http_client_get_status_code(client),
                esp_http_client_get_content_length(client));
    } else {
        ESP_LOGE(TAG, "Error perform http request %s", esp_err_to_name(err));
...
static void http_redirect_to_https(void)
{
    ...
    if (err == ESP_OK) {
        printDEBUG(DHTTP, "HTTP redirect to HTTPS Status = %d, content_length = %d",
                esp_http_client_get_status_code(client),
                esp_http_client_get_content_length(client));
    } else {
        ESP_LOGE(TAG, "Error perform http request %s", esp_err_to_name(err));
...
static void http_download_chunk(void)
{
    ...
    if (err == ESP_OK) {
        printDEBUG(DHTTP, "HTTP chunk encoding Status = %d, content_length = %d",
                esp_http_client_get_status_code(client),
                esp_http_client_get_content_length(client));
    } else {
        ESP_LOGE(TAG, "Error perform http request %s", esp_err_to_name(err));
...
static void http_perform_as_stream_reader(void)
{
    ...
    printDEBUG(DHTTP, "HTTP Stream reader Status = %d, content_length = %d",
                    esp_http_client_get_status_code(client),
                    esp_http_client_get_content_length(client));
...
//     printDEBUG(DHTTP, "Connected to AP, begin http example");
